 ...
 *
 * @uses magazinews_header_style()
 */
function magazinews_custom_header_setup() {
	add_theme_support( 'custom-header', apply_filters( 'magazinews_custom_header_args', array(
		'default-image'          => '%s/assets/uploads/custom-header-image.jpg',
		'default-text-color'     => 'fff',
		'width'                  => 1200,
		'height'                 => 528,
		'flex-height'            => true,
		'wp-head-callback'       => 'magazinews_header_style',
	) ) );
	...
	register_default_headers( array(
		'default-image' => array(
			...
			'description'   => esc_html__( 'Default Header Image', 'magazinews' ),
		),
	) );
}
add_action( 'after_setup_theme', 'magazinews_custom_header_setup' );

if ( ! function_exists( 'magazinews_header_style' ) ) :
	/**
	 * Styles the header image and text displayed on the blog.
	 *
	 * @see magazinews_custom_header_setup().
	 */
	function magazinews_header_style() {
		...
		wp_add_inline_style( 'magazinews-style', $css );
	}
endif;
add_action( 'wp_enqueue_scripts', 'magazinews_header_style', 10 );