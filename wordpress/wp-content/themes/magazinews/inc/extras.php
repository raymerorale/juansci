 ...
 * @return array
 */
function magazinews_body_classes( $classes ) {
	...
	if ( is_singular() || is_home() ) {
		$id = ( is_home() && ! is_front_page() ) ? get_option( 'page_for_posts' ) : get_the_id();
	  	$sidebar = get_post_meta( $id, 'magazinews-selected-sidebar', true );
	...
	$headline_top = $options['headline_section_show_on_top'] == false ? '' : 'headline-on-top';
	$classes[] = esc_attr($headline_top);

	return $classes;
}
add_filter( 'body_class', 'magazinews_body_classes' );
