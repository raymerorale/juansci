...
if ( ! function_exists( 'newsium_post_thumbnail' ) ) :
    /**
     ...
     */
    function newsium_post_thumbnail() {
        ...
        if ( is_singular() ) :
            ...
            $theme_class = newsium_get_option('global_image_alignment');
            $post_image_alignment = get_post_meta($post->ID, 'newsium-meta-image-options', true);
            ...
            if ( $post_class != 'no-image' ):
                ...
                <div class="post-thumbnail <?php echo esc_attr($post_class); ?>">
                    <?php the_post_thumbnail('newsium-featured'); ?>
                </div>
            <?php endif; ?>

        <?php else :
            $archive_layout = newsium_get_option('archive_layout');
            $archive_layout = $archive_layout;
            $archive_class = '';
            if ($archive_layout == 'archive-layout-list') {
                $archive_image_alignment = newsium_get_option('archive_image_alignment');
                $archive_class = $archive_image_alignment;
                $archive_image = 'medium';
            } elseif ($archive_layout == 'archive-layout-full') {
                $archive_image = 'newsium-medium';
            } else {
