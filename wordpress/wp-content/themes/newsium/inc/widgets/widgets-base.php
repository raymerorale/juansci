...
class AFthemes_Widget_Base extends WP_Widget
{
     ...
     */
    public function newsium_generate_text_input($field, $label, $value, $type = 'text', $note = '', $class = '')
    {
     ...
     * @param string $note field note to appear below
     */
    public function newsium_generate_textarea($field, $label, $note = '')
    {
     ...
     */
    public function newsium_generate_checkbox_input($field, $label, $elements, $note = '')
    {
        $instance = isset($this->form_instance[$field]) ? $this->form_instance[$field] : true;
        ?>
        <div class="newsium-multiple-check-form">
            <p>
                <label for="<?php echo esc_attr($this->get_field_id($field)); ?>">
                    <?php echo esc_html($label); ?>
                </label>
     ...
     * @param Array_A $elements
     */
    public function newsium_generate_select_options($field, $label, $elements, $note = '')
    {
    ...
    public function newsium_generate_image_upload($field, $label, $value, $note = '', $class = '')
    {
        $instance = isset($this->form_instance[$field]) ? $this->form_instance[$field] : '';
        ?>
        <div>
            ...
            <div class="image-preview-wrap">
                ...
                <input type="button" class="select-img button button-primary"
                       value="<?php esc_attr_e('Upload', 'newsium'); ?>"
                       data-uploader_title="<?php esc_attr_e('Select Image', 'newsium'); ?>"
                       data-uploader_button_text="<?php esc_attr_e('Choose Image', 'newsium'); ?>"/>
                <?php
                $image_status = false;
                if (!empty($instance)) {
                    $image_status = true;
                ...
                ?>
                <input type="button" value="<?php echo _x('X', 'Remove', 'newsium'); ?>"
    ...
    {
        $instance = array();
        $instance = $this->newsium_sanitize_data($instance, $new_instance);
        return $instance;
    }

    public function newsium_sanitize_data($instance, $new_instance)
    {
        if (is_array($this->text_fields)) {
            // update the text fields values
            foreach ($this->text_fields as $field) {
                $instance = array_merge($instance, $this->newsium_update_text($field, $new_instance));
            }
        }
        ...
        if (is_array($this->url_fields)) {
            ...
            foreach ($this->url_fields as $field) {
                $instance = array_merge($instance, $this->newsium_update_url($field, $new_instance));
            }
        }
        ...
        if (is_array($this->text_areas)) {
            ...
            foreach ($this->text_areas as $field) {
                $instance = array_merge($instance, $this->newsium_update_textarea($field, $new_instance));
            }
        }
        if (is_array($this->checkboxes)) {
            // update the checkbox fields values
            foreach ($this->checkboxes as $field) {
                $instance = array_merge($instance, $this->newsium_update_checkbox($field, $new_instance));
            }
        }
        if (is_array($this->select_fields)) {
            // update the select fields values
            foreach ($this->select_fields as $field) {
                $instance = array_merge($instance, $this->newsium_update_select($field, $new_instance));
            }
        }
        return $instance;
    }
     ...
     */
    public function newsium_update_text($name, $new_instance)
    {
        $instance = array();
     ...
     * @return object validate new instance
     */
    public function newsium_update_url($name, $new_instance)
    {
        $instance = array();
     ...
     * @return object validate new instance
     */
    public function newsium_update_textarea($name, $new_instance)
    {
        $instance = array();
     ...
     * @return object validate new instance
     */
    public function newsium_update_checkbox($name, $new_instance)
    {
        $instance = array();
     ...
     * @return object validate new instance
     */
    public function newsium_update_select($name, $new_instance)
    {
        $instance = array();
