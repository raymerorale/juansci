...
if ( ! class_exists( 'Newsium_Info' ) ) {
    ...
    class Newsium_Info {
         ...
         * Single instance.
         *
         * @var Newsium_Info $instance Instance object.
         */
        ...
        public static function init( $config ) {
            if ( ! isset( self::$instance ) && ! ( self::$instance instanceof Newsium_Info ) ) {
                self::$instance = new Newsium_Info;
        ...
        public function configure() {
            ...
            $this->menu_name     = isset( $this->config['menu_name'] ) ? $this->config['menu_name'] : sprintf( esc_html__( '%s info', 'newsium' ), $this->theme_name );
            $this->page_name     = isset( $this->config['page_name'] ) ? $this->config['page_name'] : sprintf( esc_html__( '%s info', 'newsium' ), $this->theme_name );
            $this->tabs          = isset( $this->config['tabs'] ) ? $this->config['tabs'] : array();
            $this->page_slug     = $this->theme_slug . '-details';
            $this->page_url     = admin_url( 'themes.php?page=' . $this->page_slug );
            $this->notice        = '<p>' . sprintf( esc_html__( 'Welcome! Thank you for choosing %1$s. To fully take advantage of the best our theme can offer please make sure you visit theme details page.', 'newsium' ), esc_html( $this->theme_name ) ) . '</p><p><a href="' . esc_url( $this->page_url ) . '" class="button button-primary">' . sprintf( esc_html__( 'Get started with %1$s', 'newsium' ), $this->theme_name ) . '</a><a href="#" class="btn-dismiss" data-userid="' . esc_attr( get_current_user_id() ) . '" data-nonce="' . esc_attr( wp_create_nonce( 'newsium_dismiss_nonce' ) ) . '">' . esc_html__( 'Dismiss this notice', 'newsium' ) . '</a></p>';
        }
        ...
        public function hooks() {
            ...
            // Dismiss AJAX.
            add_action( 'wp_ajax_newsium_dismiss', array( $this, 'dismiss_callback' ) );
            add_action( 'wp_ajax_nopriv_newsium_dismiss', array( $this, 'dismiss_callback' ) );
        }
        ...
        public function render_current_tab_content() {
            ...
            if ( method_exists( $this, $method ) ) {
                ...
                printf( esc_html__( '%s() method does not exist.', 'newsium' ), $method );
            }
        }
        ...
        public function useful_plugins() {
            ...
            <div class="aft-section aft-section-useful-plugins">
                ...
                <?php if ( true !== $tgmpa_complete ) : ?>
                    <a href="<?php echo esc_url( $tgmpa_url ); ?>" class="button button-primary"><?php esc_html_e( 'Manage Plugins', 'newsium' ); ?></a>
                <?php endif; ?>
                ...
                <div class="compare-table table-responsive">
                    <table class="table">
                        ...
                        <tr>
                            ...
                            <th class="db-bk-color-one"><?php _e('Product Features', 'newsium'); ?></th>
                            <th class="db-bk-color-two"><?php _e('Free', 'newsium'); ?></th>
                            <th class="db-bk-color-two"><?php _e('Premium', 'newsium'); ?></th>
                        </tr>
                        </thead>
                        <tbody>
        ...
        public function assets( $hook ) {
            ...
            if ( in_array( $hook, array( 'themes.php', 'appearance_page_' . $this->page_slug ), true ) ) {
                wp_enqueue_style( 'newsium-info', get_template_directory_uri() . '/lib/info/css/info.css', array(), '1.0.0' );
                wp_enqueue_script( 'newsium-info', get_template_directory_uri() . '/lib/info/js/info' . $min . '.js', array(), '1.0.0' );
            }
        ...
        public function display_admin_notice() {
            ...
            $dismiss_status = get_user_meta( $user_id, 'newsium_dismiss_status', true );
            ?>
        ...
        public function dismiss_callback() {
            ...
            if ( false === wp_verify_nonce( $wpnonce, 'newsium_dismiss_nonce' ) ) {
                wp_send_json( $output );
            }

            update_user_meta( $userid, 'newsium_dismiss_status', 1 );
